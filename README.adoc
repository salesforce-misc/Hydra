...
| `readTransitionAndNotifyListeners(fromState, event)` | Computes the move and fires listeners **without mutating** in-memory state — derives the state's class from the instance. (A 3-arg `(fromStateClass, fromState, event)` overload exists for when the class is known separately.) See below.
