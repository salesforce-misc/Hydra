    ...
    fun getTransitionForEvent(
      ...
      transitions.entries
        ...
        ?.let { (_, createTransitionTo) ->
          val (toState, action) = createTransitionTo(fromState, event)
          Transition.Valid(fromState, event, toState, action)
        } ?: Transition.Invalid(fromState, event)
