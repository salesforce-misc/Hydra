...
private constructor(private val machine: Machine<StateT, EventT, ActionT>) {
  ...
  fun readTransitionAndNotifyListeners(
    ...
    val transition = getMatchingTransition(fromStateClass, fromState, event)
    notifyTransition(transition, event)
    return transition
  }
  ...
  private fun getMatchingTransition(
    ...
    getState(fromStateClass)?.getTransitionForEvent(fromState, event)
      ?: Transition.NoFromState(event)

  private fun getState(fromStateClass: Class<out StateT>): Machine.State<StateT, EventT, ActionT>? =
    machine.stateDefinitions
      .filter { it.key.matches(fromStateClass) }
      .map { it.value }
      .firstOrNull()

  private fun StateT.getState() =
  ...
  private fun StateT.notifyOnEnter(cause: EventT) {
    getState()?.onEnterListeners?.forEach { it(this, cause) }
  }

  private fun StateT.notifyOnExit(cause: EventT) {
    getState()?.onExitListeners?.forEach { it(this, cause) }
  }
