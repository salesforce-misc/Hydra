...
class PersistHandler: DQHandler() {
  override fun handleEvent(eventToConsume: Event): Event? {
    info { "Consumed event: $eventToConsume to Persist Quote" }
    return persistQuote(eventToConsume as Event.Persist) // ! TODO 22/05/23 gopala.akshintala: handle if it's not the expected type 
  }
}
